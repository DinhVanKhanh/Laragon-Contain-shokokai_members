...
let savecontent = () => {
	...
	if ($.inArray($('#memberStatus').val(), ["0", "1", "2", "3"]) == -1) {
	...
	if (/^\s*$/.test(ipEx) || ipEx == "undefined") {
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				$('#submit_del').hide();
				$('#btnCloseFc').hide();
				$('.message').html(_success + data.numSuccess + "<br>" + _fail + data.numFailRows);
